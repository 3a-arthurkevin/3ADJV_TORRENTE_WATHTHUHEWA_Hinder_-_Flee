...
public class RocketScript : MonoBehaviour {
	...
	void Update () 
	{
		...
		if(Physics.Raycast( myTransform.position, myTransform.up, out rocketHit, range) && expended == false)
		{
			...
			if(rocketHit.point.y >= waterHeight - 1)
			{
			...
			{
				Instantiate(waterExplosion, rocketHit.point, Quaternion.identity);
			}
		...
		{	
			...
			if(Physics.Linecast(myTransform.position, obj.transform.position, out hit))
			{
				if(hit.transform.tag == "RedTeamTrigger")
				{
					...
					script.iWasJustAttacked = true;
					
					script.hitByRocket = true;	
					
					script.rocketDamage = damageDelivered;
				}
			}
		}
	...
	IEnumerator DestroyMySelfAfterSomeTime() 
	{
		...
		if(myTransform.position.y >= waterHeight - 1)
		{
		...
		{
			Instantiate(waterExplosion, myTransform.position, Quaternion.identity);
		}
