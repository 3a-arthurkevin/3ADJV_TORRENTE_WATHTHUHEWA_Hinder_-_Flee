...
/// 
/// This script accesses the PlayerStats script to determine the player's max
/// health.
/// 
...
/// 
/// This script is accessed by the FireParticleCannon script.
/// 
/// This script is accessed by the RocketScript.
/// 
/// This script is accessed by the Boundary script.
/// 
/// </summary>
...
public class HealthAndDamage : MonoBehaviour {
	...
	private float particleCannonDamage = 1.6f;
	...
	public bool enteredDestroyBoundary = false;
	...
	void Start () 
	{
		...
		//Access the PlayerStats script and obtain the maxHealth value.
		//This is used to set maxHealth, currentHealth and previousHealth
		//in this HealthAndDamage script across the network.
			...
			for(int i = 0; i < dataScript.PlayerList.Count; i++)
			{
				if(myAttacker == dataScript.PlayerList[i].playerName)
				{
					if(int.Parse(Network.player.ToString()) == dataScript.PlayerList[i].networkPlayer)
					{
						...
						{
							myHealth = myHealth - particleCannonDamage;
						...
						//Apply damage if the player is hit by an enemy rocket.
						...
						//If the player tried to leave the level then a fatal amount of damage is applied.
						
						if(enteredDestroyBoundary == true && destroyed == false)
						{
							myHealth = myHealth - destroyBoundaryDamage;
							
							myAttacker = "Left Level Bounds";
							
							networkView.RPC("UpdateMyCurrentAttackerEverywhere", RPCMode.Others, myAttacker);
							
							networkView.RPC("UpdateMyCurrentHealthEverywhere", RPCMode.Others, myHealth);
							
							enteredDestroyBoundary = false;
						}
						...
						if(myHealth <= 0 && destroyed == false)
						{
							...
							if(myAttacker != "" && myAttacker != "Left Level Bounds")
							{
